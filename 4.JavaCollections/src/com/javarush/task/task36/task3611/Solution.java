...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        final Set<Integer> allFriends = getAllFriends(index);
        Set<Integer> currentLevel = new HashSet<>(allFriends);

        deep = Math.max(0, deep);
        deep = Math.min(deep, humanRelationships.length - 1);
        for (int i = deep; i > 1; i--) {
            currentLevel = currentLevel.stream()
                    .map(this::getAllFriends)
                    .flatMap(Collection::stream)
                    .filter(number -> !allFriends.contains(number) && number != index)
                    .collect(Collectors.toSet());
            allFriends.addAll(currentLevel);
        }
        return allFriends;
    }
